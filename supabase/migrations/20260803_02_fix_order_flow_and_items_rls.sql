-- =====================================================================
-- Fix Order Flow and order_items RLS policies
-- =====================================================================

-- 1. Fix create_order RPC to use public.get_firebase_uid() instead of auth.uid()
...
declare
  ...
  product_row record;
begin
  current_user_id := public.get_firebase_uid();
...
using (
  exists (
    ...
    where o.id = public.order_items.order_id
      and o.user_id = public.get_firebase_uid()
  )
);
