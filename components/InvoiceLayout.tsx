...
const InvoiceLayout: React.FC<InvoiceLayoutProps> = ({ 
  ...
  const [invoiceContact, setInvoiceContact] = useState(() => ({
    supportEmail: initialSiteConfig.supportEmail || config.supportEmail || 'support@example.com',
    supportPhone: initialSiteConfig.supportPhone || config.companyPhone || '',
    websiteLabel:
      initialSiteConfig.siteUrlName ||
      initialSiteConfig.siteUrl ||
      config.domain ||
      'localhost'
  }));
  ...
  useEffect(() => {
    ...
    const applySiteConfig = (nextConfig: ReturnType<typeof getPublicSiteConfigSnapshot>) => {
      ...
      setInvoiceContact({
        supportEmail: nextConfig.supportEmail || config.supportEmail || 'support@example.com',
        supportPhone: nextConfig.supportPhone || config.companyPhone || '',
        websiteLabel: nextConfig.siteUrlName || nextConfig.siteUrl || config.domain || 'localhost'
      });
    };
    ...
    const loadSiteConfig = async () => {
      try {
        ...
        applySiteConfig(getPublicSiteConfigSnapshot());
      }
    };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12 border-b border-gray-100 pb-8 print:gap-4 print:mb-5 print:pb-4">
        <div>
          ...
          <div className="space-y-1 text-sm text-gray-500 font-medium print:text-xs">
            ...
            <p className="flex items-center gap-2">
              <MessageCircle className="w-3.5 h-3.5 text-green-600" /> {invoiceContact.supportPhone || 'N/A'}
            </p>
            <p className="flex items-center gap-2">
              <Globe className="w-3.5 h-3.5 text-blue-500" /> {invoiceContact.websiteLabel}
            </p>
          </div>
        </div>
